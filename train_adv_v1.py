...
Original file is located at
    https://colab.research.google.com/drive/1jsf30vV9hSLOiwPIOeM7vHxQD9HVBjw1
...
parser.add_argument('--rank', type=int, default = 30, help='rank of rsvd (default:30)')
parser.add_argument('--oversample', type=int, default= None, help='oversample of rsvd (default:None)')
parser.add_argument('--return_range', type=bool, default=False, help='if return range of rsvd (default: False)')
...
def aug_batch(A,rank, oversample=None, return_range=False):
    for i in range(A.shape[0]):
      A_recon[i] = np.apply_along_axis(rsvd(rank, oversample=None, return_range=False),4,A)

    return A_recon

def rsvd(A, rank, oversample=None, return_range=False):
    """
    Randomized SVD of Halko et. al. 
    A: (m x n) matrix.
    rank: target rank of approximation.
    oversample: extra columns in gaussian test matrices.
    return_range: return basis for approximate range of A.
    return: U, S, and Vt for approximate truncated SVD.
    """
    if oversample is None:
        n_samples = 2 * rank
    else:
        n_samples = rank + oversample
...
def find_range(A, n_samples):
    """
    Algorithm 4.1: Randomized range finder (of Halko et al).
    Given matrix A and a number of samples, computes an orthonormal matrix
...
def ortho_basis(M):
    """
    Computes an orthonormal basis using QR factorization for M: (m x n) matrix.
    """
    Q, _ = np.linalg.qr(M)
    return Q
